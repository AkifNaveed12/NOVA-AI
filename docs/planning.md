...
│   ├── gesture_engine.py
│   ├── hud_interface.py
│   ├── nova_hud.html          # HUD frontend (added Day 11B)
│   ├── activity_log.py
│   └── config_manager.py
├── assets/
│   └── logo/
...
> **Note:** `matplotlib` is retained for potential future data visualisation use.
> `pywebview` added at Day 11(B) — replaces `tkinter`+`matplotlib` as the HUD renderer.
> Remove `tkinter` from any explicit pip install steps — it is Python stdlib and not installed via pip.
...
- Define `INTENT_PATTERNS` dict mapping intent names to keyword lists:
  ...
  - `"datetime"`: `["time", "date", "what day", "how many days"]`
...
- `_create_tables()`: execute `CREATE TABLE IF NOT EXISTS` for all 8 tables (see architecture.md ERD)
...
## DAY 6 — Module 23 (HUD Interface — initial Tkinter prototype)

> **Status:** Superseded by Day 11(B). The Tkinter implementation was the first working HUD.
> The pywebview cinematic HUD built on Day 11(B) is the CURRENT active implementation.
> Keep this day documented for historical accuracy. Do NOT re-implement Tkinter.

**Module:** `hud_interface.py`
**Stack (original):** `tkinter`, `matplotlib`, `threading`

The original implementation built `NOVAHud` with:

- `tk.Tk()` fullscreen window (`state("zoomed")`)
- `matplotlib.animation.FuncAnimation` polar waveform (64 bars)
- `scrolledtext.ScrolledText` for activity log
- `root.after(0, fn)` for thread-safe updates from background threads

**Limitation discovered:** Fullscreen Tkinter blocked the desktop — users couldn't see apps or browser windows opening. Replaced at Day 11(B) with a pywebview floating window.

### TEST — Day 6 (historical)

- [x] HUD appeared, waveform animated, status updated, clock ticked
- [x] 0.0s latency mic fix — single shared `sr.Microphone()` stream
...
1. HUD starts in main thread (`webview.start()` after Day 11B)
...
# Test 7: HUD evaluate_js called with correct state string
#         (mock webview.create_window; verify evaluate_js("window.novaSetStatus('listening')"))
```
...
- [ ] Populate `config/apps.json` with all app paths
...
## DAY 11 (B) — Module 23 Upgrade: pywebview Cinematic HUD

> **This is the current active HUD architecture.**
> Completed immediately after Day 11. No roadmap days are skipped.

**Goal:** Replace the fullscreen Tkinter HUD (Day 6) with a cinematic, non-blocking pywebview window that renders HTML/CSS/JS. The voice pipeline, all modules, and `main.py` remain unchanged.
...
**Why pywebview was selected:**

- Native OS frameless window — no desktop obstruction
- Renders standard HTML/CSS/JS — no Python rendering code needed
- `evaluate_js()` is thread-safe — all HUD updates remain thread-safe
- Public API of `NOVAHud` is 100% identical — zero changes to `main.py`
- Single HTML file, fully offline — no web server, no ports

### T0 — Prerequisites

- [ ] `pip install pywebview`
- [ ] Verify install: `python -c "import webview; print(webview.__version__)"`

### T1 — Replace `modules/hud_interface.py`

New `NOVAHud` class uses `webview.create_window()`:

- `width=380`, `height=900`, `frameless=True`, `on_top=True`
- `background_color="#0D0D0D"`, `transparent=True`
- `window.events.loaded += self._on_loaded` — buffers JS calls until DOM ready
- `update_status(state)` → `evaluate_js("window.novaSetStatus('state')")`
- `log_message(role, text)` → `evaluate_js("window.novaAppendLog('role', text)")`
- `update_ticker(text)` → `evaluate_js("window.novaSetTicker(text)")`
- `start()` → `webview.start()` — blocks main thread exactly as Tkinter did

HTML path resolved relative to `__file__` (i.e., `modules/nova_hud.html`) — works regardless of launch directory.

### T2 — Create `modules/nova_hud.html`

Self-contained single HTML file (no CDN dependencies):

- Animated NOVA SVG logo: `pulse-ring`, `orbit`, `orbit-rev`, `breathe` CSS animations
- Canvas 2D waveform: 64 radial bars, `requestAnimationFrame` at ~60fps, state-driven
- Status bar: CSS class transitions (sleeping/listening/processing/speaking), 1Hz blink on listen
- Live JS clock via `setInterval`
- Scrolling command log: last 6 pairs, auto-scrolls
- Reminders ticker: CSS marquee animation
- Exposes globals: `window.novaSetStatus(state)`, `window.novaAppendLog(role, text)`, `window.novaSetTicker(text)`

### T3 — HUD Integration Rule (applies to all Day 12+ modules)

All modules communicate with the HUD exclusively through `main.py`'s `hud` reference:

```python
# In main.py voice_pipeline() only — never in individual modules:
...
Individual modules (email_module.py, weather.py, etc.) must NEVER import or call `hud` directly.

For reminders and calendar tickers: reminder engine puts text into `reminder_queue` (already in `main.py`) → main loop drains queue → calls `hud.update_ticker(text)`.

### T4 — Update `context.md`

### TEST — Day 11(B)

- [ ] `python main.py` starts — pywebview window appears (380px wide, always-on-top, floating)
- [ ] Desktop remains visible behind the HUD window
- [ ] Say "Hey NOVA, open Chrome" → Chrome opens in background, HUD shows command and response
- [ ] Status dot and waveform cycle: Sleeping → Listening → Processing → Speaking → Sleeping
- [ ] Clock ticks in HUD
- [ ] `hud.update_ticker("⏰ 10:00 PM — Test reminder")` → ticker scrolls

### PUSH

```bash
git add .
git commit -m "Day 11B: Module 23 upgrade — pywebview cinematic HUD, nova_hud.html, evaluate_js bridge"
git push origin main
```
...
- `main.py` handles all HUD calls — `email_module.py` returns response strings only
- Multi-turn email flow: each TTS question and user reply logs through `hud.log_message()`
...
- `save_note(content, tags="")`: INSERT into Notes table → return confirmation string
...
- `_check_loop()`: every 30 seconds, call `get_pending_reminders()`
  - For each due reminder: speak alert via TTS, mark done, put ticker text in `reminder_queue`
- `start()`: launch as `threading.Thread(daemon=True)`

> **HUD integration:** `main.py` drains `reminder_queue` and calls `hud.update_ticker(text)`.
> The reminder engine itself only puts strings into the queue — it never calls `hud` directly.
...
- `add_event(title, event_dt_str, location="", notes="")`: parse → INSERT → confirm
...
- `add_task(title, priority="medium", due_date_str=None)`: INSERT → confirm
- `get_pending_tasks()`: SELECT where is_done=False
- `mark_done(task_id_or_title)`: fuzzy match → UPDATE is_done=True
...
- In `main.py` startup: call `calendar_module.get_events_today()` → `hud.update_ticker(text)`
...
- [ ] "Add to my task list: finish NOVA project" → task stored
...
**Goal:** NOVA controls Windows system settings by voice. Takes screenshots on command.
...
- Volume: `set_volume(percent)`, `get_volume()`, `mute()`, `unmute()`, `volume_up(step=5)`, `volume_down(step=5)`
- Brightness: `set_brightness(percent)`, `brightness_up(step=10)`, `brightness_down(step=10)`
- Power: `shutdown()`, `restart()`, `sleep()`, `lock_screen()`
- Info: `get_battery()`, `get_cpu_usage()`, `get_ram_usage()`

### T2 — Implement `screenshot_tools.py`

- `take_screenshot()`: `pyautogui.screenshot()` → save to Desktop with timestamp → return filename
- `click_position(x, y)`: `pyautogui.click(x, y)`
...
- [ ] "Mute" / "Unmute" → works
- [ ] "What's my battery?" → correct percentage spoken
...
- `GestureEngine` class
- `__init__`: init MediaPipe Hands, OpenCV VideoCapture(0), `queue.Queue()`
- `_get_landmark_list(hand_landmarks)`: extract 21 (x, y) landmark positions
- `_fingers_up(landmarks)`: return 5-boolean list

### T2 — Day 18: Gesture Classification

Implement `_classify_gesture(landmarks, fingers)` — full gesture table per idea.md.

### T3 — Day 18: Debouncing System

- Gesture must hold 0.4s before triggering
- `_should_trigger(gesture)`: compare with last_gesture + time elapsed

### T4 — Day 19: Action Dispatch System

- `_dispatch_action(gesture)`: maps to pyautogui/pycaw OS actions
- Continuous volume: `math.dist(landmarks[4], landmarks[8])` → `set_volume()`

### T5 — Day 19: Main Loop + Threading

- `_gesture_loop()` at ~20 FPS: cap.read → MediaPipe → classify → debounce → dispatch
- `start()`: `threading.Thread(daemon=True).start()`
- `stop()`: set stop flag, release cap

### T6 — Day 19: HUD Note

- Gesture engine does NOT call `hud` directly
- If gesture cam feed is desired in HUD: put frame bytes into `gesture_queue` → `main.py` reads and calls a future `hud.update_gesture_cam()` method
- For now: gesture engine operates silently, HUD shows detected gestures via text label only if implemented
...
- [ ] Camera opens, hand landmarks detected without crash
- [ ] Open palm → play; fist → pause; index only → volume up
- [ ] Debounce: quick flash of gesture does NOT trigger
- [ ] Gesture engine runs at ~15-20 FPS alongside voice pipeline
...
**Goal:** Every command logged with full detail. Config manager allows hot-reload of all settings without restarting.
...
- `log(command_text, module_triggered, response_summary, success=True)`: INSERT + auto-cleanup
- `get_today()`, `get_recent(n=10)`, `cleanup_old(days=30)`

### T2 — Implement `config_manager.py`

- `ConfigManager` class
- `__init__`: load all JSON configs into memory
- `get(key, default=None)`, `set(key, value)`, `reload()`, `is_module_enabled(name)`
- `add_app(name, path)`, `add_site(name, url)`, `add_contact(name, phone, platform)`
...
- [ ] Every voice command creates an ActivityLog entry
...
- Gesture engine + voice pipeline run simultaneously
- Reminder engine fires correctly during voice interaction
- Activity log captures all commands

### T2 — Module Coverage Audit

Verify all 25 modules implemented:
...
- [x] M15 DateTime, [x] M16 System Controls, [x] M17 Screenshot
- [x] M18 Clipboard, [x] M19 Translation, [x] M20 Memory, [x] M21 Personality
- [x] M22 Gesture, [x] M23 HUD (pywebview), [x] M24 Activity Log, [x] M25 Config Manager

### T3 — Implement Module 15 as Explicit `datetime_calc.py`

- `get_time()`, `get_date()`, `days_until(target_date_str)`, `calculate(expression)`

### T4 — Implement Module 21 as Explicit `personality.py`

- `get_joke()`, `get_greeting()`, `roast(name)`
...
**Goal:** Systematic bug hunt across all 25 modules.
...
- [ ] Concurrent write from reminder engine + main thread → SQLite WAL mode

### T2 — Day 22: Module-Specific Edge Cases

**Weather:** API quota exceeded → cached last result
**Wikipedia:** Connection timeout → error without crash
...
**Email:** Gmail App Password wrong → `"Email authentication failed."`
**WhatsApp:** Session expired → `"Please log into WhatsApp Web first."`
**System Controls:** Brightness unsupported on VM → skip with warning
**Gesture Engine:** Webcam not found → disable gracefully, HUD status unaffected

### T3 — Day 23: Response Quality Polish

- [ ] All TTS responses natural, concise, < 20 words where possible
- [ ] Error messages friendly, not technical
- [ ] Startup greeting includes today's event count, pending reminders, time

### T4 — Day 23: HUD Performance Check

- [ ] pywebview window stays responsive during long Groq calls (non-blocking — already guaranteed by thread architecture)
- [ ] Log panel doesn't overflow — stays at 6 pairs max
- [ ] Ticker restarts smoothly after reminder fires
- [ ] Window remains always-on-top after browser windows open
...
```bash
git add .
git commit -m "Day 23: Polish — TTS quality, error messages, startup greeting, HUD responsiveness"
git push origin main
```
...
**Goal:** NOVA runs smoothly for extended periods.
...
| --------------------- | ----------------------------- | -------- |
| Main thread           | pywebview event loop          | Critical |
| voice_pipeline_thread | STT → NLP → route → TTS → HUD | High     |
| wake_word_thread      | pvporcupine / energy scan     | High     |
| gesture_thread        | OpenCV + MediaPipe            | Medium   |
| reminder_thread       | SQLite poll every 30s         | Low      |

- All HUD updates via `hud.update_status()` / `hud.log_message()` — `evaluate_js()` is thread-safe
- All DB writes use `check_same_thread=False` on shared connection OR per-thread connections
- No daemon thread calls `webview` API methods directly (only `NOVAHud` methods)

### T2 — Day 24: Memory Profiling

- Run NOVA for 30 minutes → check RAM with `psutil.Process().memory_info()`
- Conversation history capped at 10 messages — verify no growth
- ActivityLog cleanup runs on startup
- Gesture frame buffer not accumulating
...
- Run NOVA for 60 minutes continuous use
- Issue 50 diverse commands across all modules
- Verify: no crashes, no zombie threads, memory stable
...
```bash
git add .
git commit -m "Day 24: Threading audit — evaluate_js thread safety, DB WAL mode, gesture cleanup"
git push origin main
```
...
1. NOVA starts → cinematic pywebview HUD appears → greeting spoken
2. Wake word: "Hey NOVA, what time is it?" → HUD shows response live
3. "Hey NOVA, tell me about artificial intelligence" → Wikipedia summary

**Act 2 — Intelligence (3 min)** 4. "Hey NOVA, what's the weather in Islamabad?" → real-time weather 5. "Hey NOVA, what's in the news today?" → NASA APOD 6. "Hey NOVA, explain recursion in simple terms" → Groq explanation 7. "Hey NOVA, translate how are you to Arabic"

**Act 3 — Automation (3 min)** 8. "Hey NOVA, open VS Code" → VS Code launches (HUD visible behind it) 9. "Hey NOVA, search for Python tutorials on YouTube" → YouTube search 10. "Hey NOVA, take a screenshot" → screenshot saved 11. "Hey NOVA, set volume to 60%" → volume changes 12. Gesture demo — show open palm → play, fist → pause

**Act 4 — Memory & Productivity (2 min)** 13. "Hey NOVA, remember that my favourite IDE is VS Code" 14. "Hey NOVA, what do you know about me?" → facts recalled 15. "Hey NOVA, take a note: finish demo prep by Friday" 16. "Hey NOVA, set a reminder for 6 PM to review slides" → ticker updates

### T2 — Day 26: Demo Mode Polish

- Ensure all demo commands work reliably (test 3× each)
- Verify HUD looks impressive on screen share — pywebview window is the visual anchor
- Desktop visible behind HUD — demonstrate apps opening in real time

### T3 — Day 27: Final Documentation

- [ ] README.md: complete setup guide, all commands reference, architecture overview
- [ ] `planning.md`: confirm all days reflect actual implementation
- [ ] `context.md`: all changes documented
- [ ] `architecture.md`: Mermaid diagrams current
...
- [ ] Test fresh clone: clone → `pip install -r requirements.txt` → `python main.py`
- [ ] Add GitHub topics: `python`, `ai`, `voice-assistant`, `nlp`, `computer-vision`, `pywebview`
...
| 6     | M23      | HUD Interface (Tkinter prototype — superseded)  | `hud_interface.py`                          |
...
| 11B   | M23      | HUD Upgrade — pywebview cinematic HUD ✅ ACTIVE | `hud_interface.py`, `nova_hud.html`         |
...
- `Day 11B: Module 23 upgrade — pywebview cinematic HUD, nova_hud.html, evaluate_js bridge`
- `Day 22: Bug fixes — STT fallback, Groq backoff, DB concurrency`
...
| 7   | HUD evaluate_js called with correct state string | M23    | 7   |
...
## HUD Integration Quick Reference (Day 12+ modules)

All modules return plain strings. `main.py` owns all HUD calls.

```python
# Pattern for every module integration in main.py:
hud.update_status("processing")
response = some_module.handle(entities)         # returns a string
hud.log_message("user", original_command_text)
hud.log_message("nova", response)
speak(response)
hud.update_status("sleeping")

# Reminders/Calendar → ticker only (via queue):
reminder_queue.put("⏰ 10:00 PM — Call Ali")
# main.py drains this queue and calls hud.update_ticker(text)
```

No module file should import `webview` or reference `hud` directly.

---

_NOVA AI — Semester Project | Python 3.11 | Windows | All Free Tools_
_Planning Document v2.0 — Updated after Day 11(B) pywebview HUD migration_
